...
interface IProps {
  ...
  videoBlob: any;
  saveFinalVideo: (finalBlob: any) => void;
}
interface IState {
  img: any;
  logoX: number | string;
...
class AddLogo extends React.Component<IProps, IState> {
  ...
  componentDidMount() {
    ...
    video.src = URL.createObjectURL(this.props.videoBlob);
    ...
    this.mediaRecorder.onstop = function(e: any) {
      ...
      that.props.saveFinalVideo(blob);
      toast.success("video edited click next to upload and send");
    };
  }
  draw(
    video: any,
