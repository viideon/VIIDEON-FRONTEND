...
class Dashboard extends Component<IProps> {
  ...
  renderResponse = (tab: number) => {
    ...
    const res = step.replies?.filter((reply: any) => reply?.peopleId?._id === selectedPerson._id && reply.type === type)
    return (
      <>
  ...
  renderChoices = (choice: any, ind: number, repl: any) => {
    ...
    let isActive = false;
    replies?.filter((reply: any) => { if (reply.peopleId === selectedPerson._id) { isActive = true; } return reply });
    return (
  ...
  render() {
    const { selectedChatvid, selectedPerson, step, activeType }: any = this.state;
    return (
      <Home>
        <Grid container className="wrapperChatvidDashboard">
          ...
          <Grid item className="_responseWrapper" xs={12} sm={12} md={8} lg={8} >
            {selectedChatvid?.name &&
              (
                <Typography variant="h6"> Response on {`${selectedChatvid.name} - ${(new Date(selectedChatvid.createdAt)).toLocaleString()}`} </Typography>
              )
            }
            {selectedPerson &&
                ...
                <Grid container className="_stepsDetailsBody">
                  ...
                  {step.responseType === "Open-ended" &&
                    <Grid item xs={12} sm={2} md={2} lg={2} className="optionDiv">
                      <div className={`IconWrapper ${activeType === 1 && "activeIcon"}`} onClick={() => { this.handleTabChange(1) }}>
                        <Typography variant="h1">Tt</Typography>
                        <Typography variant="subtitle1"> Text </Typography>
                      </div>
                      <div className={`IconWrapper ${activeType === 2 && "activeIcon"}`} onClick={() => { this.handleTabChange(2) }}>
                        <VolumeUpRoundedIcon />
                        <Typography variant="subtitle1"> Audio </Typography>
                      </div>
                      <div className={`IconWrapper ${activeType === 3 && "activeIcon"}`} onClick={() => { this.handleTabChange(3) }}>
                        <VideocamRoundedIcon />
                    ...
                    {step.responseType === "Open-ended" ?
                      ...
                      step.responseType === "Multiple-Choice" ?
                        step.choices.map((choice: any, index: number) => {
                          return this.renderChoices(choice, index, step.replies)
                        }) : ""
                    }
                  </Grid>
