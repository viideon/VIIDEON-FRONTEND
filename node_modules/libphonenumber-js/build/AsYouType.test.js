...
var _AsYouType = _interopRequireWildcard(require("./AsYouType"));
...
describe('as you type', function () {
  ...
  it('should parse and format phone numbers as you type', function () {
    ...
    new AsYouType('US').input('12133734253').should.equal('1 (213) 373-4253'); // Should discard the national prefix
    ...
    var formatter; // Test International phone number (international)
    ...
    formatter.country.should.equal('US'); // This one below contains "punctuation spaces"
    ...
    type(formatter.country).should.equal('undefined');
    formatter.countryCallingCode.should.equal('1');
    type(formatter.template).should.equal('undefined'); // Check that clearing an international formatter
    ...
    formatter.country.should.equal('CH');
    type(formatter.template).should.equal('undefined'); // Kazakhstan (non-main country for +7 country phone code)

    formatter = new AsYouType();
    formatter.input('+77172580659');
    formatter.country.should.equal('KZ'); // Brazil
    ...
    formatter.country.should.equal('JE'); // Test Afghanistan phone numbers

    formatter = new AsYouType('AF'); // No national prefix

    formatter.input('44444444').should.equal('44444444');
    type(formatter.template).should.equal('undefined'); // With national prefix

    formatter.reset().input('044444444').should.equal('044 444 444');
    formatter.template.should.equal('xxx xxx xxxx'); // Braces must be part of the template.

    formatter = new AsYouType('RU');
    formatter.input('88005553535').should.equal('8 (800) 555-35-35');
    formatter.template.should.equal('x (xxx) xxx-xx-xx'); // Test Russian phone numbers
    ...
    new AsYouType('BG').input('111 222 3').should.equal('1112223'); // Brazil
    ...
    new AsYouType('BR').input('123456789').should.equal('12 3456-789'); // Deutchland

    new AsYouType().input('+4915539898001').should.equal('+49 15539 898001'); // KZ detection

    formatter = new AsYouType();
    formatter.input('+7 702 211 1111');
    formatter.country.should.equal('KZ'); // formatter.valid.should.equal(true)
  ...
  it('should not forgive incorrect international phone numbers', function () {
    var formatter;
    formatter = new AsYouType(); // formatter.input('+1 1 877 215 5230').should.equal('+1 1 877 215 5230')

    formatter.input('+1 1 877 215 5230').should.equal('+1 18772155230'); // formatter.getNationalNumber().should.equal('8772155230')
  ...
  it('should close non-paired braces', function () {
    (0, _AsYouType.closeNonPairedParens)('(000) 123-45 (9  )', 15).should.equal('(000) 123-45 (9  )');
  });
  it('should strip non-paired braces', function () {
    (0, _AsYouType.stripNonPairedParens)('(000) 123-45 (9').should.equal('(000) 123-45 9');
    (0, _AsYouType.stripNonPairedParens)('(000) 123-45 (9)').should.equal('(000) 123-45 (9)');
  });
  it("should fall back to the default country", function () {
    ...
    formatter.country.should.equal('RU'); // formatter.countryCallingCode.should.equal('7')
  ...
  it('should work in edge cases', function () {
    ...
    formatter.input('+');
    type(formatter.country).should.equal('undefined');
    type(formatter.countryCallingCode).should.equal('undefined'); // Country not inferrable from the phone number,
    // while the phone number itself can already be formatted "completely".

    formatter = new AsYouType();
    formatter.input('+12223333333');
    type(formatter.country).should.equal('undefined');
    formatter.countryCallingCode.should.equal('1'); // An otherwise matching phone number format is skipped
    ...
    formatter.reset().input('1010000').should.equal('10 1000 0'); // Reset a chosen format when it no longer applies given the new leading digits.
  ...
  it('should work with Argentina numbers', function () {
    ...
    asYouType.reset(); // Digits shouldn't be changed.
  ...
  it('should work with Mexico numbers', function () {
    ...
    asYouType.getTemplate().should.equal('xxx xx xxxx xxxx'); // https://issuetracker.google.com/issues/147938979
  ...
  it('should return PhoneNumber', function () {
    var formatter = new AsYouType('RU');
  ...
  it('should get separator after national prefix', function () {
    ...
    formatter.getSeparatorAfterNationalPrefix(format).should.equal(' '); // Britain.
